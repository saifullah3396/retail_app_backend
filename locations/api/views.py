...
class LocationsListCreateDestroyView(CoreListCreateDestroyView):
    ...
    def _filter_locations_by_id_list(self, locations, id_list):
        """
        Filters the locations by id list. If all ids in the list do not
        match, a not found exception is raised.
        """
        filtered_locations = filter_queryset_by_id_list(
            locations, id_list)
    ...
    def _get_organization_admin_queryset(self):
        ...
        if id_list:
            return self._filter_locations_by_id_list(
