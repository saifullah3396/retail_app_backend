...
from app_organizations.dels import (DefaultOrganizationGroups,
                                    OrganizationGroup)
from app_organizations.sts.factories import organization_group_factory
...
def generate_fake_group_data():
    ...
    """
    data = generate_fake_data(organization_group_factory.UserGroupFactory)
    data['authority'] = generate_fake_data(
        organization_group_factory.UserGroupFactory)['authority']
    return data


class OrganizationGroupsListCreateDestroyTests(TestsBase):
    """
    Defines unit tests for 'list/create/destroy' api requests for views
    defined at 'organizations/groups/' url.
    ...
    def generate_factory_data(self):
        ...
        self.org_groups = \
            organization_group_factory.generate_org_org_groups_factory(
                self.orgs.values())
    ...
    def list_org_groups_response_check_fn(self, user, response, *args, **kwargs):
        """
    ...
    def generate_data_for_id_list_groups(self):
        ...
        id_list.extend(
            organization_group_factory.OrganizationGroupFactory.create_batch(5))

        return [('name', group.name) for group in id_list]

    def id_list_org_groups_response_check_fn(self, user, response, query_params, *args, **kwargs):
        """
    ...
    def create_group_response_check_fn(self, user, response, data, *args, **kwargs):
        ...
        """
        if user.is_superuser:
            if data['authority'] >= DefaultOrganizationGroups.FREE_USER.value:
    ...
    def list_destroy_org_groups_response_check_fn(self, response, *args, **kwargs):
        """
    ...
    def id_list_destroy_org_groups_response_check_fn(self, user, response, *args, **kwargs):
        """
        ...
        self.tests = [
            {
                ...
                'type': 'post',
                'path_name': 'org_groups_list_create_destroy',
                'request': [
                    {
            ...
            #         {
            #             'name': 'list_groups',
            #             'response_check_fn': self.list_org_groups_response_check_fn,
            #         },
            #     ]
            # },
            # {
            ...
            #             'response_check_fn': self.id_list_org_groups_response_check_fn,
            #         }
            #     ]
            # },
            # {
            ...
            #             'response_check_fn': self.list_destroy_org_groups_response_check_fn,
            #         },
            #     ]
            # },
            # {
            ...
            #             'response_check_fn': self.id_list_destroy_org_groups_response_check_fn,
            #         },
            #     ]
            # }
        ]
